...
//* MISION GENERAL DE ESTE ARCHIVO app.js:
/* LEVANTA AL SERVIDOR EXPRESS Y SE CONECTA A LA BASE DE DATOS SQLITE */
//Recibe las peticiones HTTP y responde a ellas
// y conecta a la base de datos SQLite(busca, elimina, etc)
// Sirve datos en formato JSON para que el frontend los consuma
...
const app = express();   //app es la instancia de express que levanta el servidor
const port = 3000; // Puerto para el backend, donde express va a escuchar las peticiones

// Middlewares
app.use(cors()); // Habilita CORS, quien permite conexiones desde cualquier origen (frontend)
app.use(express.json()); // Permite a Express leer JSON en las peticiones entrantes

/*Hasta aca*/


/* Hacer solo si no me dan una base de datos */
//Verifica si la base de datos de libros está vacía y, si es así, inserta datos iniciales
...
//RUTAS (ENDPOINTS) DE LA API RESTFUL
...
//Inicializa el servidor y base de datos

(async function main() {
    try {
        await sequelize.authenticate();  // Verifica que la conexión a la base de datos funcione
        console.log("✔ Conexión establecida con la base de datos");
        await sequelize.sync({ force: true }); // o { alter: true }   Borra y vuelve a crear las tablas
